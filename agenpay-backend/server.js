 ...
 * Express.js server with authentication, Notion integration, CDP wallets, and AI agents
 */
...
// 📊 Wallet Status & CDP Info
app.get('/api/wallet/status', authenticateToken, async (req, res) => {
  try {
    const cdpStatus = walletService.getStatus();
    const hasWallet = await walletService.hasWallet(req.user.id);
...
app.post('/api/wallet/send', authenticateToken, async (req, res) => {
  try {
    const { toAddress, amount, currency = 'ETH', network = 'base-sepolia' } = req.body;
...
- Wallet: ${walletService.initialized ? '✅ CDP Live' : '🧪 Mock Mode'}
...
- POST /api/wallet/create - Create CDP wallet
