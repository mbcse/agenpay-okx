...
export class AgenPayAgent {
  constructor(userId, userConfig = {}, sharedServices = {}) {
    ...
    this.config = {
      ...
      defaultNetwork: process.env.DEFAULT_NETWORK || 'base-sepolia',
  ...
  setupTools() {
    ...
    try {
      ...
      // Add create multi-token payment link tool
      this.createMultiTokenPaymentLink = tool(
        async ({ paymentData, acceptedTokens = ['ETH', 'USDC', 'USDT', 'WETH'] }) => {
          console.log(`💱 [User ${this.userId}] Creating multi-token X402Pay payment link...`);
          console.log(`🎯 [User ${this.userId}] Preferred receive currency: ${paymentData.currency}`);
          console.log(`🪙 [User ${this.userId}] Accepted payment tokens: ${acceptedTokens.join(', ')}`);
          
          try {
            // Create multi-token X402Pay payment link using OKX DEX integration
            const paymentLink = await this.x402PayService.createMultiTokenPaymentRequest({
              userId: this.userId,
              amount: paymentData.amount,
              preferredReceiveCurrency: paymentData.currency,
              acceptedPaymentTokens: acceptedTokens,
              network: paymentData.network,
              recipientEmail: paymentData.recipientEmail,
              recipientName: paymentData.recipientName,
              description: paymentData.description,
              transactionType: paymentData.transactionType || 'ask_payment',
              scheduleType: paymentData.scheduleType || 'immediate',
              scheduledDate: paymentData.scheduledDate,
              aiPrompt: paymentData.aiPrompt
            });

            console.log(`✅ [User ${this.userId}] Multi-token payment link created:`, paymentLink);
            console.log(`🔗 [User ${this.userId}] PAYMENT LINK: ${paymentLink.x402PayLink}`);
            console.log(`💰 [User ${this.userId}] PREFERRED RECEIVE: ${paymentData.amount} ${paymentData.currency}`);
            console.log(`🪙 [User ${this.userId}] ACCEPTS: ${acceptedTokens.join(', ')}`);
            console.log(`📊 [User ${this.userId}] OKX DEX SUPPORT: ${paymentLink.supportedChains} chains, ${paymentLink.supportedTokens} tokens`);
            
            return {
              success: true,
              paymentLink: paymentLink.x402PayLink,
              paymentId: paymentLink.paymentId,
              multiTokenEnabled: paymentLink.multiTokenEnabled,
              preferredReceiveCurrency: paymentLink.preferredReceiveCurrency,
              acceptedPaymentTokens: paymentLink.acceptedPaymentTokens,
              supportedChains: paymentLink.supportedChains,
              supportedTokens: paymentLink.supportedTokens,
              message: `Multi-token payment link created! Accepts ${acceptedTokens.join(', ')} → ${paymentData.currency}. Link: ${paymentLink.x402PayLink}`
            };
          } catch (error) {
            console.error(`❌ [User ${this.userId}] Multi-token payment link creation failed:`, error);
            
            // Fallback to regular payment link if OKX DEX fails
            console.log(`🔄 [User ${this.userId}] Falling back to regular payment link...`);
            try {
              const fallbackLink = await this.x402PayService.createPaymentRequest({
                userId: this.userId,
                amount: paymentData.amount,
                currency: paymentData.currency,
                description: `${paymentData.description} (OKX DEX unavailable - standard payment)`,
                recipientEmail: paymentData.recipientEmail,
                network: paymentData.network
              });
              ...
              return {
                ...
                fallbackMode: true,
                message: `Payment link created (standard mode): ${fallbackLink.x402PayLink}`
              };
            } catch (fallbackError) {
              return {
                success: false,
                error: fallbackError.message,
                message: 'Failed to create payment link (both multi-token and standard modes failed)'
              };
            }
          }
        },
        {
          ...
          description: 'Create a multi-token X402Pay payment link that accepts various cryptocurrencies with automatic swapping via OKX DEX',
          schema: z.object({
            paymentData: z.object({
              amount: z.string(),
              currency: z.string().describe('Preferred currency to receive (e.g., USDC, ETH)'),
              description: z.string(),
              recipientEmail: z.string(),
            ...
            acceptedTokens: z.array(z.string()).optional().describe('Array of token symbols to accept for payment (default: [ETH, USDC, USDT, WETH])')
          })
        }
      );
      ...
      this.getPaymentOptions = tool(
        async ({ paymentId, userPreferredToken = 'ETH' }) => {
        ...
        },
        {
          ...
          schema: z.object({
            paymentId: z.string(),
            userPreferredToken: z.string().optional().describe('Token the user wants to pay with (default: ETH)')
          })
        }
      );
...
- Multi-token payments accept ETH, USDC, USDT, WETH and automatically swap to the recipient's preferred currency
