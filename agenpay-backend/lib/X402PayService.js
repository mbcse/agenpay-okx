...
export class X402PayService {
  ...
  setupRoutes() {
    ...
    this.app.use('/x402pay/:paymentId', async (req, res, next) => {
      ...
      // Get user's specific CDP wallet address
      ...
      if (!user?.walletAddress) {
        return res.status(400).json({ 
          error: 'User wallet not found. Please create a CDP wallet first.' 
        });
      }
      ...
      const middlewareConfig = {
        "/": {  // Use "/" instead of full path since Express strips the matched part
          ...
          network: paymentConfig.network || "base-sepolia",
          config: {
      ...
      try {
        ...
        const middleware = paymentMiddleware(
          user.walletAddress, // Use user's specific CDP wallet
          middlewareConfig,
          this.facilitatorConfig
        );
  ...
  async createPaymentRequest({
    ...
    try {
      ...
      if (!user?.walletAddress) {
        throw new Error('User must create a CDP wallet before requesting payments');
      }
  ...
  async createMultiTokenPaymentRequest({
    ...
    try {
      ...
      // Get supported chains and tokens from OKX DEX
      const supportedChains = await this.okxDexService.getSupportedChains();
      const supportedTokens = await this.okxDexService.getTokensForChain(network);

      if (!supportedChains.success || !supportedTokens.success) {
        console.warn(`⚠️ [${this.instanceId}] OKX DEX API unavailable, falling back to standard payment`);
        // Fallback to regular payment request
      ...
      // Generate unique payment ID
      const paymentId = `x402_multi_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      
      // Create the payment URL
      const paymentUrl = `${process.env.BASE_URL || 'http://localhost:3001'}/x402pay/${paymentId}`;
      
      // Create payment request in database with multi-token support
      const paymentRequest = await this.prisma.paymentRequest.create({
        data: {
          id: paymentId,
          userId,
          ...
          recipientEmail,
          recipientName,
          description: `${description} (Multi-token payment - OKX DEX enabled)`,
          aiPrompt,
          transactionType,
          scheduleType,
          scheduledDate: scheduledDate ? new Date(scheduledDate) : null,
          ...
          metadata: JSON.stringify({
            ...
            preferredReceiveCurrency,
            acceptedPaymentTokens,
            okxDexEnabled: true,
            supportedChains: supportedChains.data?.length || 0,
            supportedTokens: supportedTokens.data?.length || 0
          })
        }
      });

      // Store payment configuration in memory for X402 middleware with multi-token support
      console.log(`🔧 [${this.instanceId}] Adding multi-token payment route to memory: ${paymentId}`);
      ...
      const routeConfig = {
        ...
        acceptedPaymentTokens,
        preferredReceiveCurrency,
        okxDexService: this.okxDexService,
        createdAt: new Date()
      };
      
      this.paymentRoutes.set(paymentId, routeConfig);
      
      console.log(`✅ [${this.instanceId}] Multi-token X402 payment request created: ${paymentId}`);
      console.log(`💰 [${this.instanceId}] Amount: ${amount} ${preferredReceiveCurrency} (accepting: ${acceptedPaymentTokens.join(', ')})`);
      console.log(`🎯 [${this.instanceId}] Destination: ${user.walletAddress}`);
      console.log(`🗺️ [${this.instanceId}] Payment route added to memory. Total routes: ${this.paymentRoutes.size}`);
  ...
  /**
   * Calculate payment options for multi-token request
   */
  async calculatePaymentOptions(paymentId, userPreferredToken = 'ETH') {
    try {
      ...
      if (!routeConfig.multiTokenEnabled) {
        return {
          swapRequired: false,
          directPayment: true,
      ...
      // Get token addresses for the preferred payment token and receive currency
      const chainConfig = this.okxDexService.supportedChains[routeConfig.network];
      if (!chainConfig) {
        throw new Error(`Unsupported network: ${routeConfig.network}`);
      }

      const preferredTokenAddress = this.okxDexService.popularTokens[routeConfig.network]?.[userPreferredToken] || 
                                   chainConfig.nativeToken;
      const receiveCurrencyAddress = this.okxDexService.popularTokens[routeConfig.network]?.[routeConfig.preferredReceiveCurrency] || 
                                    chainConfig.usdc;

      // Calculate optimal payment route
      const paymentRoute = await this.okxDexService.calculatePaymentRoute({
        payerPreferredToken: preferredTokenAddress,
        payeeRequiredToken: receiveCurrencyAddress,
        amount: routeConfig.amount.toString(),
        chainId: chainConfig.chainId,
        userWalletAddress: routeConfig.userWalletAddress
      });

      return {
        paymentId,
        userPreferredToken,
        receiveCurrency: routeConfig.preferredReceiveCurrency,
        ...paymentRoute
      };
    ...
    try {
      ...
      await this.prisma.transaction.create({
        data: {
          ...
          toAddress: user?.walletAddress, // User's CDP wallet that received payment
